...
rollapply_func <- function(roll_window, next_window,w) {
  ...
  n_simulations <- m
  simulated_copula <- rCopula(n_simulations, copula_fit$beta_copula)
  ...
  inf_rc=which(is.infinite(simulated_returns), arr.ind = TRUE)
  
  ##remove bad simulated r copula which are mostly when sgt.mle doesnot converge
  if(any(is.infinite(simulated_returns))){
    simulated_returns=simulated_returns[-inf_rc[,1],]
    warning(paste0("rolling window has some infty rcopula samples",w))
  }
  count_inf_rc=length(inf_rc[,1])
  
  # Optimize portfolio copula_cov method and data_cov
  optimized_portfolio <- portfolio.opt(simulated_returns,roll_window, next_window)
  
  Sys.time()-s1
  ## next day return performance measures
  result_copula_cov=next_day(optimized_portfolio$weights_copula_cov_3_constr,
                             roll_window = roll_window ,
                             next_window =next_window )
  
  result_copula_cov1=next_day(optimized_portfolio$weights_copula_cov_2_constr,
                             roll_window = roll_window ,
                             next_window =next_window )
  
  result_data_cov=next_day(optimized_portfolio$weights_data_cov_3_constr,
                           roll_window = roll_window ,
                           next_window =next_window )
  ...
  return(list(copula_cov_3_constr= result_copula_cov,
              copula_cov_2_constr= result_copula_cov1,
...
## for dev ** uncomment above if u run over cluster
w=1 #i=52#i=201 sgt.mle is problematic if method=nlminb 
...
if (rebal_step == 1) {
  ...
  next_window <- returns_matrix[(start_idx + rebal_step):(end_idx + rebal_step), ]
  #next_window <- returns_matrix[(end_idx + 1):(end_idx + rebal_step), ]
}
